...
A test case can be represented in either of the following formats:
	...
	// To interact with clients, you can launch them using the t.simClient method:
	//
	//    c := t.simClient()
	//    c.RPC().Call(...)
	//
	...
	type ClientTestSpec struct {
		...
		Run         func(*T, *simClient) // this is the function that will be executed by the test suite
	}
...
The `T` object can start a client using the `StartClient()` method. `StartClient()` returns an object `simClient` with
information about the client container. `simClient` also offers two methods: `EnodeURL()`, which returns the enode URL
